...
/// Available commands for caddy-gen
...
enum Command {
    ...
    /// Initialize caddy-gen by setting up folders to import Caddyfile.dev from
    Init,
...
/// Get the caddy-gen config directory (~/.config/caddy-gen)
...
fn generate_caddyfile_dev(
    ...
    println!("Reload Caddy with: caddy-gen reload");
}

/// Interactive initialization to set up import folders
fn init_caddygen() {
    println!("=== Caddy-gen Initialization ===");
    ...
    caddyfile_content.push_str("# Auto-generated by caddy-gen init\n");
    caddyfile_content.push_str("# Edit this file or run 'caddy-gen init' to reconfigure\n\n");
    ...
    println!("Run 'caddy-gen reload' to apply the configuration.");
}
...
fn reload_caddy() {
    ...
    if !main_caddyfile_path.exists() {
        ...
        eprintln!("Run 'caddy-gen init' first to set up the configuration.");
        std::process::exit(1);
    }
